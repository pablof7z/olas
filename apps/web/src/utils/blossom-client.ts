...
export class BlossomClient {
    ...
    // upload blob
    async getUploadAuth(message = 'Upload Blob', sha256: string, expiration?: number) {
        if (!this.signer) throw new Error('Missing signer');
        return await BlossomClient.getUploadAuth(this.signer, message, sha256, expiration);
    }
    async uploadBlob(file: UploadType, auth: SignedEvent | boolean = true, sha256?: string) {
        if (typeof auth === 'boolean' && auth) {
            ...
            auth = await this.getUploadAuth('Upload Blob', sha256);
        }
        return BlossomClient.uploadBlob(this.server, file, auth === true ? undefined : auth);
    }
    ...
    async mirrorBlob(url: string | URL, auth: SignedEvent | boolean = true, sha256?: string) {
        if (typeof auth === 'boolean' && auth) {
            if (!sha256) throw new Error('Missing sha256');
            auth = await this.getMirrorAuth(sha256);
        }
        return BlossomClient.mirrorBlob(this.server, url, auth === true ? undefined : auth);
    }
    ...
    async deleteBlob(hash: string, auth: SignedEvent | boolean = true) {
        ...
        return BlossomClient.deleteBlob(this.server, hash, auth === true ? undefined : auth);
    }
} 