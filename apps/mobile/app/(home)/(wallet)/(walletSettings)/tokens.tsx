...
import { NDKCashuToken, NDKCashuMintList, NDKKind, useNDKCurrentUser, useNDKSessionEventKind, useNDKWallet, useSubscribe } from "@nostr-dev-kit/ndk-mobile";
...
function InspectButton({ token }: { token: NDKCashuToken }) {
    ...
    if (inspect) {
        return (<Modal visible={inspect} onDismiss={() => setInspect(false)}>
            <SafeAreaView>
    ...
    return <Button variant="tonal" onPress={() => setInspect(true)}>
        <Text>Inspect</Text>
    </Button>
}
...
function TokenItem({ tokenId, token, mint, proofEntries }: { tokenId: string, token?: NDKCashuToken, mint: string, proofEntries: ProofEntry[] }) {
    ...
    const handlePublish = async () => {
        ...
        setStatusUpdate(res ? "Published to " + Array.from(res).map(r => r.url).join(", ") : "Failed");
    };

    const handleValidate = async () => {
        setStatusUpdate("Validating...");
        consolidateMintTokens(mint, activeWallet as NDKCashuWallet, proofEntries.map(p => p.proof),
            (res) => {
    ...
    return <Swipeable renderRightActions={() => <RightActions event={token} />}>
        <View style={[itemStyles.container, { backgroundColor: colors.card }]} className="flex-col gap-4 p-2">
            <Pressable className="flex-1 flex-row items-center justify-between gap-2" onPress={() => setOpen(isOpen ? null : tokenId)}>
                ...
                <View className="flex-1 flex-col">
                    ...
                    <Text className="text-muted-foreground text-xs">{relays.size} relays</Text>
                </View>
            ...
            {isOpen && (
                <View className="flex-col">
                    {Array.from(relays).map(relay => <Text key={relay} className="text-muted-foreground text-sm">{relay}</Text>)}

                    {showProofs && (
                        <>
                            {proofEntries.map(({ proof }) => <View className="flex-row" key={proof.C}>
                                <Text className="flex-1 text-muted-foreground text-sm">Proof {proof.C.substring(0, 10)}</Text>
                    ...
                    <Text className="text-muted-foreground text-xs">{token?.encode() ?? tokenId}</Text>
                </View>
            )}
        </View>
    </Swipeable>
    ...
    return (
        <View style={rightActionStyles.container}>
            ...
            <Pressable style={[rightActionStyles.button, rightActionStyles.validate]} onPress={handleValidate}>
                ...
                <Text className="text-white text-xs">Validate</Text>
            </Pressable>
        </View>
    )
}
...
function ForceSync() {
    ...
    ], { wrap: true, skipVerification: true, closeOnEose: true })
...
function ForceSyncItem({ mint, proofs }: { mint: string, proofs: { amount: number, proof: any }[] }) {
    ...
    const startValidation = useCallback(async () => {
        setValidating(true);
        await consolidateMintTokens(mint, activeWallet, proofs.map(p => p.proof),
            (res) => {
    ...
    return <View key={mint}>
        <Text>{mint}: {proofs.length} coins</Text>
        {result ? <Text>{result}</Text> : (
            <>
                {validating ? <ActivityIndicator /> : <Button variant="outline" onPress={startValidation}>
                    <Text>Validate</Text>
                </Button>}
            </>
        )}
