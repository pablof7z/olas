...
export function MediaPreview({
    ...
    const containerStyle = useMemo<ViewStyle>(() => {
        if (multiple)
            return { flex: 1, width: size - 25, overflow: 'hidden', borderRadius: 20, marginLeft: 5, marginRight: 5, marginVertical: 2 };
    ...
    return (
        <Container>
            {mediaToRender.map((item, index) => (
                <View
                    key={item.id}
                    className="flex-1 flex-row items-stretch justify-stretch"
                    style={{ width: maxWidth, height: maxHeight }}>
                    {item.mediaType === 'video' ? (
                        <VideoAlbumItem uri={mediaUri} style={{ flex: 1, width: '100%', height: '100%' }} />
                    ) : (
                        <View style={{ ...containerStyle }}>
                            <PhotoAlbumItem key={item.id} uri={mediaUri} style={containerStyle} resizeMode="contain" />

                            {withEdit && (
                                <View className="absolute bottom-0 left-0 right-0 m-2 flex-row items-stretch gap-1 rounded-full">
                                    {item.uris.length > 1 && (
                                        <Pressable
                                            onPress={() => handleRevertPress(index)}
                                            className="flex-row items-center gap-4 rounded-full bg-black/50 px-4 py-3 ">
...
export function VideoAlbumItem({ uri, style, ...props }: { uri: string; style: ViewStyle } & ComponentProps<typeof View>) {
    ...
    return (
        <VideoView
            ...
            style={style}
            className="w-full flex-1 flex-col items-stretch justify-stretch bg-red-500"
            {...props}
        />
    );
}
