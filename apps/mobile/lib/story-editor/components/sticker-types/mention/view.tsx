import type { NDKStoryStickerType, NDKUser } from '@nostr-dev-kit/ndk-mobile';
import { LinearGradient } from 'expo-linear-gradient';
import React, { useState, useCallback } from 'react';
import { type LayoutChangeEvent, StyleProp, StyleSheet, type TextStyle, View } from 'react-native';

import mentionStyles, {
    type ExtendedViewStyle,
    getStyleFromName,
    type MentionStickerStyle,
} from './styles';
...
export default function MentionStickerView({
    sticker,
    fixedDimensions = false,
    onLayout,
}: MentionStickerViewProps) {
    // Initialize containerSize with dimensions from sticker if fixedDimensions is true, otherwise empty
    const [containerSize, setContainerSize] = useState(
        fixedDimensions ? sticker.dimensions : { width: 0, height: 0 }
    );
