...
export default function AlbumsGrid({
    albums,
    onAlbumPress
}: { albums: MediaLibrary.Album[], onAlbumPress: (album: MediaLibrary.Album) => void }) {
    ...
    const albumsWithAssets = albums.filter((album) => assetsPerAlbum[album.id]?.totalCount > 0 && assetsPerAlbum[album.id]?.assets.length > 0);
...
export function AlbumListItem({
    ...
    containerStyle,
    onPress
}: { album: MediaLibrary.Album, asset: MediaLibrary.Asset, totalCount: number, imageStyle?: ImageStyle, containerStyle?: ViewStyle, onPress: () => void }) {
    ...
    return <Pressable style={containerStyle} onPress={onPress}>
        <Image source={{ uri: asset.uri }} style={imageStyle} />
        <Text variant="subhead" className="text-foreground">{album.title}</Text>
        <Text variant="caption2" className="text-muted-foreground">{totalCount}</Text>
    </Pressable>
}