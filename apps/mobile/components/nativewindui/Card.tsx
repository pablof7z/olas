...
const Card = React.forwardRef<React.ElementRef<typeof View>, React.ComponentPropsWithoutRef<typeof View> & { rootClassName?: string }>(
    ({ className, rootClassName, ...props }, ref) => (
        <View className={cn('ios:shadow-xl ios:rounded-2xl rounded-xl bg-card shadow-2xl', rootClassName)}>
            <View ref={ref} className={cn('ios:rounded-2xl justify-end overflow-hidden rounded-xl', className)} {...props} />
        </View>
    )
);
Card.displayName = 'Card';

const CardBadge = React.forwardRef<React.ElementRef<typeof View>, React.ComponentPropsWithoutRef<typeof View>>(({ className, style, ...props }, ref) => {
...
>(({ className, linearGradientColors, iosBlurIntensity = 3, iosBlurClassName, ...props }, ref) => {
    if (linearGradientColors) {
        return (
            <LinearGradient colors={linearGradientColors ?? []} className="pt-4">
                {Platform.OS === 'ios' && (
                    <BlurView intensity={iosBlurIntensity} className={cn('absolute bottom-0 left-0 right-0 top-1/2', iosBlurClassName)} />
                )}
    ...
    return (
        <>
            {Platform.OS === 'ios' && <BlurView intensity={iosBlurIntensity} className={iosBlurClassName} />}
...
const CardImage = React.forwardRef<React.ElementRef<typeof Image>, Omit<ImageProps, 'className'> & { materialRootClassName?: string }>(
    ({ transition = 200, style = StyleSheet.absoluteFill, contentPosition = 'top right', contentFit = 'cover', materialRootClassName, ...props }, ref) => {
        ...
        return (
            <Root
                ...
                <Image ref={ref} transition={transition} style={style} contentPosition={contentPosition} contentFit={contentFit} {...props} />
            </Root>
        );
    }
);
...
function CardTitle({ className, ...props }: React.ComponentPropsWithoutRef<typeof Text>) {
    return (
        <Text
            role="heading"
            aria-level={3}
            className={cn('ios:font-bold text-3xl font-medium leading-none tracking-tight text-card-foreground', className)}
            {...props}
        />
    );
}

function CardSubtitle({ className, variant = Platform.select({ ios: 'footnote' }), ...props }: React.ComponentPropsWithoutRef<typeof Text>) {
    return <Text variant={variant} className={cn('ios:font-bold ios:uppercase font-medium opacity-70', className)} {...props} />;
}
...
const CardFooter = React.forwardRef<React.ElementRef<typeof BlurView>, React.ComponentPropsWithoutRef<typeof BlurView>>(({ className, ...props }, ref) => (
    <BlurView
        ref={ref}
...
export { addOpacityToRgb, Card, CardBadge, CardContent, CardDescription, CardFooter, CardImage, CardSubtitle, CardTitle };
