...
export function MediaPreview({ assets, style, containerStyle }: { assets: MediaLibraryItem[], style?: ImageStyle, containerStyle?: ViewStyle }) {
    ...
    if (assets.length === 1) {
        ...
        return (<View className="w-full flex-1" style={contentStyle}>
...
export function VideoAlbumItem({ uri, style, ...props }: { uri: string, style: ViewStyle } & ComponentProps<typeof View>) {
    ...
    return <VideoView player={player} style={style} className="flex-1 flex-col items-stretch justify-stretch w-full" {...props} />
}