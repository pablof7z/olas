import { DbImageCacheEntry } from './types';
 ...
 */
export async function upsertImageCacheEntry(
  entry: DbImageCacheEntry
): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    writeQueue.push({
      sql: `INSERT OR REPLACE INTO image_cache (original_url, fetched_url, width, state, filesystem_key, attempts)
            VALUES (?, ?, ?, ?, ?, ?)`,
      params: [
        entry.originalUrl,
...
export async function markImageCacheFailure(
  ...
  return new Promise<void>((resolve, reject) => {
    writeQueue.push({
      sql: `
        INSERT OR REPLACE INTO image_cache (original_url, width, state, filesystem_key, attempts)
        VALUES (
          ?,
          ?,
          'error',
      ...
      `,
      params: [originalUrl, width, filesystemKey, originalUrl, width],
      resolve,
      reject,
    });
    scheduleFlush();
...
export async function getAllImageCacheEntries(): Promise<DbImageCacheEntry[]> {
  ...
  const query = `SELECT original_url as originalUrl, fetched_url as fetchedUrl, width, state, filesystem_key as filesystemKey, attempts FROM image_cache`;
  try {
